import fsPromises from 'fs/promises'; // 使用 promises API，并将导入名称更改为 fsPromises
...
export async function generateEthWallet(num = 10) {
  ...
  const walletfile = `./data/wallet/walletEth-${currentTime}.csv`; // 生成文件名

  // 判断文件是否存在
  try {
    await fsPromises.access(walletfile);
  } catch {
    // 文件不存在则创建文件并写入标题行
    const header = 'indexId,ethAddress,enEthPrivateKey,enEthMnemonic\n';
    await fsPromises.writeFile(walletfile, header);
  }

  const file = await fsPromises.open(walletfile, 'a');
...
export async function generateBtcWallet(num = 10) {
  ...
  const currentTime = getCurrentTime();
  const walletfile = `./data/wallet/walletBtc-${currentTime}.csv`; // 生成文件名
...
export async function generateSolWallet(num = 10) {
  const currentTime = getCurrentTime();
  const walletfile = `./data/wallet/walletSol-${currentTime}.csv`; // 生成文件名
...
export async function generateSuiWallet(num = 10) {
  const currentTime = getCurrentTime();
  const walletfile = `./data/wallet/walletSui-${currentTime}.csv`; // 生成文件名
...
export async function generatePassword(num = 10) {
  const currentTime = getCurrentTime();
  const walletfile = `./data/wallet/walletPassword-${currentTime}.csv`; // 生成文件名
